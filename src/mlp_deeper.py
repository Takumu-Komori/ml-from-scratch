...
class MLP(torch.nn.Module):
  def __init__(self, d_in: int, d_hidden: int, d_out: int, depth: int, activation: str ="relu"):
    super().__init__()
    ...
    self.layers.append(torch.nn.Linear(d_in, d_hidden))
    for _ in range(depth - 2):
      self.layers.append(torch.nn.Linear(d_hidden, d_hidden))
    self.out = torch.nn.Linear(d_hidden, d_out)
...
def main():
    ...
    p.add_argument("--activation", type=str, default="relu", choices=["relu", "sigmoid", "tanh", "none"])
    ...
    for step in range(1, args.steps + 1):
        ...
        if step % 10 == 0 or step == 1:
          ...
          log_rows.append([step, float(loss.item()), g_first, g_last])
